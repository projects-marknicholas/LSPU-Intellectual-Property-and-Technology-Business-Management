...
$previous_image = $target_dir . $branch . '-cover.webp';
...
if(isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
    $target_file = $target_dir . $branch . '-cover.webp';
    $filename = $branch . '-cover.webp';
    ...
    if (move_uploaded_file($_FILES["photo"]["tmp_name"], $target_file)) {
        ...
        // Update branch_cover data based on the user's branch
        $sql = "UPDATE branches 
                SET branch_cover = '$filename' 
                WHERE branch_name = '$branch'";

        if ($link->query($sql) === TRUE) {
            $response['status'] = 'success';
            $response['message'] = 'Branch cover updated successfully';
            $response['filename'] = $filename;
        } else {
            $response['message'] = 'Error updating branch cover: ' . $link->error;
        }
    } else {
