...
$sql = "SELECT branch_profile FROM branches WHERE branch_name = '$branch'";
$result = $link->query($sql);

if ($result && $result->num_rows > 0) {
    $row = $result->fetch_assoc();
    $branchProfileImage = $row['branch_profile'];

    if (empty($branchProfileImage) || is_null($branchProfileImage)) {
        $branchProfileImage = 'main-logo.png';
    }

    $response['status'] = 'success';
    $response['message'] = $branchProfileImage;
} else {
    $response['message'] = 'Error retrieving branch profile image: ' . $link->error;
}
