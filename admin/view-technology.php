...
<html>
<head>
	<title>View Technology | RTMS</title>
	<meta charset="UTF-8">
...
<section id="rtms">
	...
	<div class="main-panel">
		...
		<div id="target">
			...
			<form method="post" action="#" id="update-form" class="record-form-grid">
			    ...
			    <div class="record-year record-item">
			        <label for="year">Year<br>
			            <input type="number" min="1900" max="2099" id="year" step="1" value="<?php echo date('Y')?>" name="year" />
			        </label>
			    </div>
			    ...
			    <div class="record-button record-item">
			    	<label for="button">Update<br></label>
			        <button id="button">Update Changes</button>
			    </div>
			</form>
			<!-- End -->
		</div>
...
$(document).ready(function () {
    ...
    function fetchTechnologyById() {
        var techId = <?php echo isset($_GET['tech_id']) ? $_GET['tech_id'] : 'null'; ?>;

        if (techId !== null) {
            $.ajax({
                url: '../access/admin/view-technology',
                type: 'GET',
                data: { tech_id: techId },
                dataType: 'json',
                success: function (data) {
                    // Populate form fields with fetched data
                    $('#technology').val(data.message.technology);
                    $('#ip').val(data.message.ip_type);
                    $('#year').val(data.message.year);
                    $('#date-of-filing').val(data.message.date_of_filing);
                    $('#application-no').val(data.message.application_no);
                    $('#record-status').val(data.message.technology_status);
                	var abstractText = $('<div>').html(data.message.abstract).text();
					var inventorsText = $('<div>').html(data.message.inventors).text();
					$('#abstract').summernote('code', abstractText);
					$('#inventors').summernote('code', inventorsText);

                    // Display image preview (replace 'preview-record-img' with the actual ID of your preview element)
                    $('#preview-record-img').attr('src', '../access/files/records/' + data.message.banner_img);
                },
            ...
            console.error('Invalid technology ID.');
        }
    }

    // Function to update technology field in real-time
    function updateTechnologyField(fieldName, fieldValue) {
        var techId = <?php echo isset($_GET['tech_id']) ? $_GET['tech_id'] : 'null'; ?>;

        if (techId !== null) {
            $.ajax({
                url: '../access/admin/update-technology',
                type: 'POST',
                data: { tech_id: techId, field_name: fieldName, field_value: fieldValue },
                success: function (response) {
                	
                },
                error: function (xhr, status, error) {
                    $('#validation').html('<div class="validation-handler error-handle"><img src="../assets/svg/error.svg"><span class="danger">' + err +'</span></div>');
                }
            });
        } else {
            console.error('Invalid technology ID.');
    ...
    $('#update-form').submit(function (event) {
        ...
        updateTechnologyField('technology', $('#technology').val());
        updateTechnologyField('ip_type', $('#ip').val());
        updateTechnologyField('year', $('#year').val());
        updateTechnologyField('date_of_filing', $('#date-of-filing').val());
        updateTechnologyField('application_no', $('#application-no').val());
        updateTechnologyField('technology_status', $('#record-status').val());
        updateTechnologyField('abstract', $('#abstract').summernote('code'));
        updateTechnologyField('inventors', $('#inventors').summernote('code'));
    });

    // Initial fetch of technology by ID
    fetchTechnologyById();
});
